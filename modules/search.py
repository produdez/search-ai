...
def ilterativeDeepeningSearch(problem,print_state=False):
    depth = 0
    while (1):
        print('Iterative search with max depth: ', depth)
...
def stochasticHillClimb(problem,heuristic, print_state = False):
    #init node
    current_node = SearchNode(problem.getStartState(),0 ,heuristic(problem.getStartState()))
...
def genericSearch(problem,frontier = None, print_steps = False, limit = None, heuristics = nullHeuristic):
    ...
    start_node = SearchNode(start_state,0,heuristics(start_state))
                    ...
                    for i, node in enumerate(explored):
                        if node == child_node:
                            ...
                            if node.better(child_node):
                                ...
                                if print_steps: print(f'Replaced in explored old: {node} with {child_node}')
